...
use crate::workers::news_stories::phase_2::news_story_llm_category_summary::process_single_item::process_single_item;
...
/// Project scraped, summarize the categories.
pub async fn news_story_llm_category_summary_main_loop(job_state: Arc<JobState>) {
  loop {
    debug!("news_story_llm_category_summary_main_loop main loop");

    single_job_loop_iteration(&job_state).await;

    debug!("llm_category_summary loop finished; waiting...");
