...
use crate::workers::news_stories::phase_2::news_story_image_generation::process_single_item::process_single_item;
...
use sqlite_queries::queries::by_table::news_story_productions::list::list_news_story_productions_awaiting_image_generation::list_news_story_productions_awaiting_image_generation;
use std::sync::Arc;
use std::thread;
use std::time::Duration;

// TODO: Stop copy/pasting this. Come up with a generalizable task runner with dependencies, etc.

/// Generate images for the news story
pub async fn news_story_image_generation_main_loop(job_state: Arc<JobState>) {
  loop {
    debug!("image generation main loop");

    single_job_loop_iteration(&job_state).await;

    debug!("image generation finished; waiting...");
...
async fn rendition_jobs_of_status(status: AwaitableJobStatus, job_state: &Arc<JobState>) {
  ...
  loop {
    ...
    debug!("image generation querying {:?} targets from id > {} ...", &status, last_id);

    let query_result = list_news_story_productions_awaiting_image_generation(
