...
pub async fn news_story_audio_preprocessing_main_loop(job_state: Arc<JobState>) {
  loop {
    debug!("news_story_audio_preprocessing main loop");

    query_and_process_all_batches(&job_state).await;

    debug!("news_story_audio_preprocessing loop finished; waiting...");
...
async fn query_and_process_all_batches(job_state: &Arc<JobState>) {
  ...
  loop {
    ...
    debug!("querying targets from id > {} ...", last_id);

    let query_result = list_news_story_productions_awaiting_audio_generation(
    ...
    for target in targets {
      debug!("Split audio for target: {:?}", target.original_news_canonical_url);
