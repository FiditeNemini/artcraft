...
pub fn extract_text(document: &Html, text_selector: &Selector) -> Option<String> {
  let text_element = match document.select(text_selector).next() {
    None => return None,
    Some(text_element) => text_element,
  };

  let mut pieces = Vec::new();

  for mut text in text_element.text() {
    text = text.trim();
    if !text.is_empty() {
