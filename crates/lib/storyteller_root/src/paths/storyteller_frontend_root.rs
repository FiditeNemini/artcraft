...
/// The env var that declares where the storyteller frontend monorepo lives.
pub const STORYTELLER_FRONTEND_ROOT : &str = "STORYTELLER_FRONTEND_ROOT";

// DO NOT LEAK THIS. THIS IS FOR TESTING ONLY.
pub const TEST_STORYTELLER_FRONTEND_ROOT : &str = "ENV_TEST_STORYTELLER_FRONTEND_ROOT_DO_NOT_LEAK_";

/// Get the root of the Rust monorepo
pub fn get_storyteller_frontend_root() -> PathBuf {
  // 1) Try "STORYTELLER_FRONTEND_ROOT" env var override. Do not check for path existence.
  let root = if cfg!(test) { TEST_STORYTELLER_FRONTEND_ROOT } else { STORYTELLER_FRONTEND_ROOT };
...
mod tests {
  ...
  use crate::paths::storyteller_frontend_root::{get_storyteller_frontend_root, TEST_STORYTELLER_FRONTEND_ROOT};

  #[test]
  #[serial]
  fn test_storyteller_frontend_home_env_var() {
    ...
    std::env::set_var(TEST_STORYTELLER_FRONTEND_ROOT, "/testing/storyteller/root/custom-storyteller-frontend");
    assert_eq!(get_storyteller_frontend_root(), PathBuf::from("/testing/storyteller/root/custom-storyteller-frontend"));
    std::env::remove_var(TEST_STORYTELLER_FRONTEND_ROOT);
  }
  ...
  fn test_relative_to_home_directory_no_directories_exist() {
    ...
    assert_eq!(get_storyteller_frontend_root(), PathBuf::from("/testing/fake_home/code/storyteller/storyteller-frontend"));
  ...
  fn test_relative_to_home_directory_with_existing_directory() {
    ...
    expected.push("storyteller-frontend");

    assert_eq!(get_storyteller_frontend_root(), expected);
  ...
  fn test_relative_to_current_working_directory() {
    ...
    expected_dir.push("code/storyteller/storyteller-frontend");

    assert_eq!(get_storyteller_frontend_root(), expected_dir);
  }
}
