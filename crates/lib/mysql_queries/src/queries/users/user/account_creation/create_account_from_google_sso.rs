...
pub enum CreateAccountFromGoogleSsoError {
  EmailIsTaken,
  UsernameIsTaken,
  DatabaseError,
  OtherError,
...
) -> Result<UserToken, CreateAccountFromGoogleSsoError>
{
...
SET
  ...
  let _record_id = match query_result {
    ...
    Err(err) => {
          ...
          match maybe_code.as_deref() {
            Some("23000") => {
              if err.message().contains("username") {
                return Err(CreateAccountFromGoogleSsoError::UsernameIsTaken);
              } else if err.message().contains("email_address") {
                return Err(CreateAccountFromGoogleSsoError::EmailIsTaken);
              }
            }
            _ => {},
          }
      ...
      return Err(CreateAccountFromGoogleSsoError::DatabaseError);
    }
  };
