...
fn raw_record_to_public_result(record: RawGenericInferenceJobStatus) -> GenericInferenceJobStatus {
  let maybe_model_title = match record.inference_category {
    ...
    InferenceCategory::ConvertFbxToGltf => Some("FBX to GLTF"),
  };
  ...
  let (mut bucket_path_is_hash, mut maybe_public_bucket_hash) = match record.inference_category {
    ...
    InferenceCategory::ConvertFbxToGltf => (true, record.maybe_media_file_public_bucket_directory_hash.as_deref()),
  };
