...
impl FromRow<'_, MySqlRow> for RawWeightJoinUser {
  fn from_row(row: &MySqlRow) -> Result<Self, sqlx::Error> {
    Ok(Self {
      ...
      private_bucket_hash: row.try_get("private_bucket_hash")?,
      maybe_private_bucket_prefix: row.try_get("maybe_private_bucket_prefix")?,
      maybe_private_bucket_extension: row.try_get("maybe_private_bucket_extension")?,
