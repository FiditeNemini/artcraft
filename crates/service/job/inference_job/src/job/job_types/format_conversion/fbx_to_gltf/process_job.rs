...
pub async fn process_job(args: FbxToGltfJobArgs<'_>) -> Result<JobSuccessResult, ProcessSingleJobError> {
  ...
  info!("Saving record (only gltf, not the buffer.bin) ...");
  ...
  let (inference_result_token, id) = insert_media_file_generic(InsertArgs {
    ...
    maybe_mime_type: maybe_gltf_mimetype.as_deref(),
    file_size_bytes: gltf_file_size_bytes,
    ...
    public_bucket_directory_hash: result_gtlf_bucket_location.get_object_hash(),
    maybe_public_bucket_prefix: result_gtlf_bucket_location.get_optional_prefix(),
    maybe_public_bucket_extension: result_gtlf_bucket_location.get_optional_extension(),
    ...
    checksum_sha2: &gltf_file_checksum,
