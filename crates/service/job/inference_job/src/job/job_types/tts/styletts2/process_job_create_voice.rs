...
/// We'll increment this if we change the underlying StyleTTS2 code in an incompatible way.
const CURRENT_STYLE_TTS2_MODEL_VERSION: u64 = 0;
...
pub async fn process_create_voice(
  ...
  let embedding_bucket_location = ZeroShotVoiceEmbeddingBucketPath::generate_new(
    ModelCategory::Tts,
    ModelType::StyleTTS2,
    CURRENT_STYLE_TTS2_MODEL_VERSION
  );
  ...
  let voice_token = create_voice(CreateVoiceArgs {
    ...
    model_type: ZsVoiceModelType::StyleTTS2,
...
mod tests {
  ...
  use crate::job::job_types::tts::styletts2::process_job_create_voice::join_paths;
