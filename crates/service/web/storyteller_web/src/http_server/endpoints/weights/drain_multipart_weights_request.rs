...
pub async fn drain_multipart_request(mut multipart_payload: Multipart) -> AnyhowResult<WeightUploadRequest> {
  let mut uuid_idempotency_token = None;
  let mut model_weight_token = None;
  ...
  while let Ok(Some(mut field)) = multipart_payload.try_next().await {
    ...
    match field_name.as_deref() {
      ...
      Some("dataset_token") => {
        model_weight_token = read_multipart_field_as_text(&mut field).await
            .map_err(|e| {
