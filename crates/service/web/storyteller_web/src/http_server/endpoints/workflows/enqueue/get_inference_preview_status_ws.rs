...
pub async fn get_inference_preview_status_handler_ws(
  http_request: HttpRequest,
  path: Path<GetInferenceJobStatusPathInfo>,
  server_state: Data<Arc<ServerState>>) -> Result<Json<GetInferenceJobStatusSuccessResponse>, GetInferenceJobStatusError>
{
...
pub async fn ws_progress_route(
  req: HttpRequest,
  stream: web::Payload,
  srv: web::Data<Addr<progress_tracker_server::ChatServer>>,
) -> Result<HttpResponse, Error> {
  ws::start(
    progress_tracker_session::WsChatSession {
      id: 0,
      hb: Instant::now(),
      room: "main".to_owned(),
      name: None,
