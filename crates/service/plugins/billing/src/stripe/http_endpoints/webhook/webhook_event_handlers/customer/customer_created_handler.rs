...
pub fn customer_created_handler(customer: &Customer) -> Result<StripeWebhookSummary, StripeWebhookError> {
  ...
  let maybe_user_token = customer.metadata
      .as_ref()
      .and_then(|m| m.get(METADATA_USER_TOKEN).clone().map(|t| t.to_string()));
