...
use crate::deps::Deps;
use crate::migrations::tts_models_to_weights::copy_cloud_files::copy_cloud_files;
...
async fn copy_model_record_and_bucket_files(record: &WholeTtsModelRecord, deps: &Deps) -> AnyhowResult<()> {
  ...
  if let Some(copied_data) = &maybe_copied_data {
    ...
    upsert_model_weight_from_tts_model(record, &deps.mysql_production, copied_data).await?;
  }
...
async fn lookup_bucket_details_from_record(model_weight_token: &ModelWeightToken, deps: &Deps) -> AnyhowResult<Option<CopiedTtsFileData>> {
  ...
  let bucket_path = match model_weight_record.weights_type {
    WeightsType::Tacotron2 => WeightFileBucketPath::tt2_model_file_from_object_hash(&model_weight_record.public_bucket_hash),
