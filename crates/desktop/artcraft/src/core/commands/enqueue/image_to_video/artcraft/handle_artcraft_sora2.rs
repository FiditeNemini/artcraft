...
pub (super) async fn handle_artcraft_sora2(
  ...
  let aspect_ratio = match request.sora_orientation {
    Some(SoraOrientation::Portrait) => Sora2MultiFunctionVideoGenAspectRatio::NineBySixteen,
    Some(SoraOrientation::Landscape) => Sora2MultiFunctionVideoGenAspectRatio::SixteenByNine,
    None => Sora2MultiFunctionVideoGenAspectRatio::SixteenByNine,
  };
  ...
  let request = Sora2MultiFunctionVideoGenRequest {
    ...
    resolution: Some(Sora2MultiFunctionVideoGenResolution::SevenTwentyP), // TODO: Parameterize
    duration: Some(Sora2MultiFunctionVideoGenDuration::TwelveSeconds), // TODO: Parameterize
  };
  
  let result = sora_2_multi_function_video_gen(
