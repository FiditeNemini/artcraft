...
export class AnimationClip {
  ...
  update_bones() {
    ...
    if(rootObject)
    for (let index_ = 0; index_ < (rootObject as THREE.Object3D<THREE.Object3DEventMap>).children.length; index_++) {
      const child_holder = (rootObject as THREE.Object3D<THREE.Object3DEventMap>).children[index_];
      if (child_holder.type == "Bone") {
        child_holder.traverse((bone: THREE.Object3D<THREE.Object3DEventMap>) => {
          for (let index__ = 0; index__ < this.special_properties.length; index__++) {
            ...
            if (property.bone == bone.name + ".quaternion") {
              ...
              if(property.only_y == false){
                ...
                if(child_holder.parent){
                  child_holder.parent.rotation.x = THREE.MathUtils.degToRad(-90);
                }
              }
              ...
              bone.position.set(bone.position.x*property.x, bone.position.y*property.y, bone.position.z*property.z);
            }
          }
        });
      }
