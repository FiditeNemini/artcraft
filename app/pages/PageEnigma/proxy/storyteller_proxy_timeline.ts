...
export class StoryTellerProxyTimeline {
    ...
    private async getItemsDict(items: { [key: string]: any }): Promise<{ [key: string]: any }> {
    ...
    private async loadTransformClips(items: { [key: string]: any }): Promise<{ [key: string]: any }> {
        let timeline_items_data: { [key: string]: any } = {};
        for (const key in items) {
            if (items.hasOwnProperty(key)) {
                const element = items[key];
                timeline_items_data[key] = new TransformClip(element.version, element.object_uuid, element.length, element.media_id);
    ...
    private async loadAudioClips(items: { [key: string]: any }): Promise<{ [key: string]: any }> {
        let timeline_items_data: { [key: string]: any } = {};
        for (const key in items) {
            if (items.hasOwnProperty(key)) {
                const element = items[key];
                timeline_items_data[key] = new AudioClip(element.version, element.media_id, element.volume);
            }
        }
        return timeline_items_data;
    }

    private async loadAnimationClips(items: { [key: string]: any }): Promise<{ [key: string]: any }> {
        let timeline_items_data: { [key: string]: any } = {};
        for (const key in items) {
            if (items.hasOwnProperty(key)) {
                const element = items[key];
                timeline_items_data[key] = new AnimationClip(element.version, element.media_id, "remote", element.object_uuid, element.speed, element.length, element.clip_name);
            }
        }
        return timeline_items_data;
    }

    private async loadLipsyncClips(items: { [key: string]: any }): Promise<{ [key: string]: any }> {
        let timeline_items_data: { [key: string]: any } = {};
        for (const key in items) {
            if (items.hasOwnProperty(key)) {
                const element = items[key];
                timeline_items_data[key] = new LipSyncClip(element.version, element.media_id, element.volume);
            }
        }
        return timeline_items_data;
    }
