...
impl Config {
  pub async fn load(secrets: Secrets) -> Self {
    let a1 = Config::load_inner(secrets.clone()).await;
    if let Some(conf) = a1 {
      return conf;
    }

    Config::load_inner(secrets.clone()).await.unwrap()
  }
  ...
  async fn load_inner(secrets: Secrets) -> Option<Self> {
    ...
    // TODO: DO NOT COMMIT
    let consumer_key = &secrets.api_key;
    let consumer_secret = &secrets.api_secret_key;


    println!("Trying to get connection to Twitter - creating con_token");
    ...
    if true {
      ...
      let accessTokenSecret = &secrets.access_token_secret;

      username = secrets.username.to_string();
      user_id = secrets.user_id;

      let access_token = egg_mode::KeyPair::new(
        accessToken.to_string(),
        accessTokenSecret.to_string(),
      );
