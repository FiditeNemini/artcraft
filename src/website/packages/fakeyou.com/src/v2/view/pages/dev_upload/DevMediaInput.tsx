...
import { useModal, useNotifications,
  // useSession 
} from "hooks";
import { InferenceJobsModal } from "components/modals";
import { Button, Checkbox, NumberSlider, SegmentButtons, TempInput, TempSelect } from "components/common";
...
export default function DevMediaInput({ value }: Props) {
  ...
  const createNotification = () => create({
    actions,
    autoRemove,
    title: notTitle,
    content: notContent,
  ...
  return (<div {...{ className: "fy-engine-compositor p-3"}}>
    ...
    <header>
      ...
      <NumberSlider {...{ value: btnCount, name: "btnCount", onChange, label: "Button amount", min: 0, max: 3 }}/>
      <div {...{ className: "lower-controls" }}>
        <Checkbox {...{ checked: autoRemove, name: "autoRemove", onChange, label: "Auto remove" }}/>
        <Button {...{ label: "Create Notification", onClick: createNotification, variant: "primary" }}/>
      </div>
    </div>
