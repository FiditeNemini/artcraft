...
pub fn validate_github_username(username: &str) -> Result<(), String> {
  lazy_static! {
...
mod tests {
  ...
  #[test]
  fn valid_cases() {
    assert!(validate_github_username("echelon").is_ok());
    assert!(validate_github_username("a-b-c-D-E-F-0-1-2").is_ok());
  }
