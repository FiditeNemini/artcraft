...
#COPY --from=build /tmp/target/debug/twitter-ingestion-client /
COPY --from=build /tmp/target/release/twitter-ingestion-client /
...
RUN ldd twitter-ingestion-client

EXPOSE 8080
CMD LD_LIBRARY_PATH=/usr/lib /twitter-ingestion-client
