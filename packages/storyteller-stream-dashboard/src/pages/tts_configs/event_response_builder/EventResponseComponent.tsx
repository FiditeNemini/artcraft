...
function EventResponseComponent(props: EventResponseComponentProps) {
  ...
  const [eventResponseType, setEventResponseType] = useState<EventResponseType>(
    EventResponseType.TtsSingleVoice
  );

  // ========== Cached Values for Editing ==========

  const [selectedTtsModelTokens, setSelectedTtsModelTokens] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (!!props.serverEventResponse.tts_single_voice) {
      ...
      if (!!props.serverEventResponse.tts_single_voice.tts_model_token) {
        modelTokens.push(
          props.serverEventResponse.tts_single_voice.tts_model_token
        );
      }
      ...
      if (
        !!props.serverEventResponse.tts_random_voice.tts_model_tokens &&
  ...
  const handleChangedEventResponseType = (
    ev: React.FormEvent<HTMLSelectElement>
  ): boolean => {
    const value = (ev.target as HTMLSelectElement).value;
    const newResponseType = value as EventResponseType;

    const newEventResponse = buildEventResponse(
      selectedTtsModelTokens,
      newResponseType
    );
  ...
  return (
    <>
      <div className="panel p-3 p-lg-4">
        <h2 className="fw-bold pb-3">3) Pick how to respond</h2>

        <div className="form-group">
          <label className="sub-title">Response Type</label>
          <div className="control">
            ...
            >
              <option value={EventResponseType.TtsSingleVoice}>
                Respond with a TTS voice
              </option>
              <option value={EventResponseType.TtsRandomVoice}>
                Respond with a Random TTS voice from list
              </option>
            </select>
          </div>
        </div>
      ...
      <div className="panel p-3 p-lg-4">
        <h2 className="fw-bold pb-3">4) Configure the response</h2>
        {responseForm}
      </div>
    </>
  );
