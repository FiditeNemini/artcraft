...
import {
  ...
  cheerStateToPredicate,
  predicateToCheerState,
} from "./CheerState";
...
function EventMatchPredicateBuilderComponent(
  props: EventMatchPredicateBuilderComponentProps
) {
  // ========== Core UI flow ==========

  const [bitsRuleType, setBitsRuleType] = useState<BitsRuleType>(
    BitsRuleType.BitsCheermoteNameExactMatch
  );
  const [channelPointsRuleType, setChannelPointsRuleType] =
    useState<ChannelPointsRuleType>(
      ChannelPointsRuleType.ChannelPointsRewardNameExactMatch
    );
    ...
    switch (props.twitchEventCategory) {
      case TwitchEventCategory.Bits:
        if (!!props.serverEventMatchPredicate.bits_cheermote_name_exact_match) {
          ...
          !!props.serverEventMatchPredicate
  ...
  const handleChangedBitsRuleType = (
  ...
  const handleChangedChannelPointsRuleType = (
    ev: React.FormEvent<HTMLSelectElement>
  ): boolean => {
    const value = (ev.target as HTMLSelectElement).value;
  ...
  const handleChangedCheerName = (name: string) => {
    ...
    if (!!maybeCheerPrefix) {
      ...
      if (CheerStateIsCustom(cheerState)) {
        let { cheerPrefix, bitValue } = CheerUtil.parseCheerString(
          cheerState.cheerFull || ""
        );

        if (
          !!cheerPrefix &&
          !!bitValue && // We have a complete name
          bitValue !== cheerState.bits
        ) {
          // The bit value is not independent of the cheer name, as in eg. (Foobar1000, 1000)
          // Keep the old value.
  ...
  if (props.twitchEventCategory === TwitchEventCategory.Bits) {
    ruleTypeSelect = (
      <>
        ...
        >
          <option value={BitsRuleType.BitsSpendThreshold}>
            Bits Spend Threshold
          </option>
          <option value={BitsRuleType.BitsCheermoteNameExactMatch}>
            Cheermote Name (Exact Match)
          </option>
          <option value={BitsRuleType.BitsCheermotePrefixSpendThreshold}>
            Cheermote Prefix and Bits Spend Threshold
          </option>
        </select>
      </>
    );
    ...
    ruleTypeSelect = (
      <>
        ...
        >
          ...
          >
            Channel Points Reward Name (exact match)
          </option>
        </select>
      </>
    );
    ...
    matchingRulesForm = (
      <ChannelPointsRewardNameExactMatchForm
  ...
  return (
    <>
      <div className="d-flex flex-column gap-4">
        <div className="d-flex flex-column panel p-3 p-lg-4 gap-2">
          <h2 className="fw-bold pb-3">1) Pick what to match on</h2>
          <div className="form-group">{ruleTypeSelect}</div>
        </div>

        <div className="d-flex flex-column panel p-3 p-lg-4 gap-2">
          <h2 className="fw-bold pb-3">2) Configure the matching</h2>
          <div className="form-group">{matchingRulesForm}</div>
        </div>
      </div>
    </>
  );
