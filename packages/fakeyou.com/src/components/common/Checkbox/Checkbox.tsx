...
export default function Checkbox({ checked, className = "", label = "", name = "", onChange, variant = "secondary" }: Props) {
  const onClick = ({ target }: any) => onChange({ target: { checked: !checked, name, type: 'checkbox' } });
  const id = "checkbox-" + useId();
  return <div {...{
    ...makeClass(`fy-checkbox-frame fy-checkbox-${ variant }-${ checked ? "checked" : "unchecked" }`,className),
  }}>
