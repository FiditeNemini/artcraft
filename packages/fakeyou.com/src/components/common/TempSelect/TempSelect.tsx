...
export default function Select({ label, icon, name, onChange: inChange = () => {}, options = [], rounded, small, value, ...rest }: SelectProps) {
  ...
  return <>
    ...
    <div className={`form-group${ icon ? " input-icon" : "" }${ small ? " select-small" : "" }`}>
      { icon && <FontAwesomeIcon icon={icon} className="form-control-feedback" /> }
      <div className="w-100">
        <ReactSelect{...{ classNamePrefix: "select", classNames, name, onChange, options, value: { label: valueLabel, value }, ...rest }}/>
      </div>
    </div>
  </>;
};