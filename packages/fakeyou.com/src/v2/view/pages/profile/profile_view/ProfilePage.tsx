...
function ProfilePage(this: any, props: Props) {
   ...
   const Comments = () => <div className="mt-3 mt-lg-0">
      <CommentComponent { ...{ entityType: 'user', entityToken: userData?.user_token, sessionWrapper: props.sessionWrapper }}/>
   </div>;
   ...
   const tabs = [
      ...
      { Component: ProfileW2lTemplateListFc, val: 'uploadedw2l', txt: 'Uploaded Templates' },
   ...
   const tabLinks = tabs.map(({ val, txt },key) => {
      ...
      return <li {...{ className: 'nav-item', key, role: 'presentation' }}>
         <Link {...{
            'aria-controls': val,
   ...
   const BaseRoute = ({ baseUrl }: { baseUrl?: string }) => {
      return <Route {...{ path: baseUrl || url }}>
         <motion.div {...{ 'aria-labelledby': 'ttsresults-tab', className: 'tab-pane fade show active', id: 'ttsresults', role: 'tabpanel', variants: panel }}>
   ...
   const tabPanels = tabs.map(({ Component, txt, val },key) => {
      ...
      return !key ? BaseRoute({ baseUrl: subRoute }) : <Route {...{  key, path: subRoute }}>
         <motion.div {...{ 'aria-labelledby': `${val}-tab`, id: 'val', role: 'tabpanel', variants: panel }}
